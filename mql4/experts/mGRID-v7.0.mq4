...
int main() {
   int    ticket, profit, total, PipsLot;
   double InitialPrice, spread=(Ask-Bid)/Point;
   ...
   if (AccountMargin() > 0.0) MarginLevel = 100.0 * (AccountEquity() / AccountMargin());
   else                       MarginLevel = 0.0;
   MarginLevelStr = DoubleToStr(MarginLevel, 2);

   if (pips < MarketInfo(Symbol(), MODE_STOPLEVEL)+spread)
      pips = 1 + MarketInfo(Symbol(), MODE_STOPLEVEL) + spread;
   ...
   if (TotalTrades+TotalOrders == 0 && total < 1 && Enter && (!UseEntryTime || (UseEntryTime && Hour()==EntryTime))) {
      // Set up a new grid
      InitialPrice = Ask;
      ...
      for (cpt=0; cpt < total; cpt++) {
         ...
         if (OrderMagicNumber()==Magic && OrderSymbol()==Symbol() && StrToDouble(OrderComment())==InitialPrice) {
            EndSession();
            return(0);
         }
      }
...
void CountOrdersAndTrades() {
   ...
   for (cpt = 0; cpt < OrdersTotal(); cpt++) {
      ...
      if (OrderMagicNumber()==Magic && OrderSymbol()==Symbol()) {
         ...
         else if (OrderType() == OP_SELL    ) NumSells++;
         else if (OrderType() == OP_BUYSTOP ) NumBuyOrders++;
         else if (OrderType() == OP_SELLSTOP) NumSellOrders++;
      }
   }
