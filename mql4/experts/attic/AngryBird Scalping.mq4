...
#property link      ""
// -----------------------------------------------------------------------------------------------------------------------------------------
double Stoploss = 500.0;            // break-even level
double TrailStart = 10.0;
double TrailStop = 10.0;
// -----------------------------------------------------------------------------------------------------------------------------------------
extern double LotExponent = 2;   // how much to multiply the lot when the next knee is set. example: first lot 0.1, series: 0.16, 0.26, 0.43 ...
extern bool    DynamicPips                   = true;
extern int     DefaultPips                   = 12;
extern int Glubina = 24;
extern int DEL = 3;
extern double slip = 3.0;           // on how much the price may differ in the event that the DC requests requotes (at the last moment, it will slightly change the price)
extern double Lots = 0.01;          // lot size for bidding start
extern int lotdecimal = 2;          // how many signs after the comma in the lot to calculate 0 - normal lots (1), 1 - minilots (0.1), 2 - micro (0.01)
extern double TakeProfit = 20.0;    // when many profit points are reached, close the deal
//extern double PipStep = 30.0;     // step between putting up new knees
extern double Drop = 500;
extern double RsiMinimum = 30.0;    // lower bound of RSI
extern double RsiMaximum = 70.0;    // upper limit of RSI
extern int MagicNumber = 2222;      // magic number (helps the advisor to distinguish his bets from others)
int PipStep=0;
// -----------------------------------------------------------------------------------------------------------------------------------------
...
extern double MaxTradeOpenHours = 48.0;    // the timeout of transactions in hours (how much to close the suspended transactions)
// -----------------------------------------------------------------------------------------------------------------------------------------
