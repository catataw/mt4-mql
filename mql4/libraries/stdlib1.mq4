...
// Sommerzeit-Umschaltzeiten für GMT/BST (London) GMT+0000,GMT+0100
datetime BST_schedule[50][4] = {
   // Umschaltzeiten in GMT/BST                       // Umschaltzeiten in GMT
...
int GetTerminalTopWindow() {
   ...
   int error = GetLastError();
   if (error != ERR_NO_ERROR) {
      last_library_error = catch("GetTerminalTopWindow()", error);
      return(0);
   }
   return(child);
}
...
int SetWindowText(int hWnd, string text) {
   ...
   if (!SetWindowTextA(hWnd, text)) {
      ...
      if (error == ERR_NO_ERROR)
         error = ERR_WINDOWS_ERROR;
      last_library_error = catch("SetWindowText()   user32.SetWindowTextA(hWnd="+hWnd+", lpString=\""+text+"\")    result: 0", error);
      return(error);
   }
