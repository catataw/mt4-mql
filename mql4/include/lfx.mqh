...
bool LFX.ReadRemotePosition(int account, int ticket, int &orderType, double &orderUnits, string &orderSymbol, datetime &openTime, double &openPrice, double &stopLoss, double &takeProfit, datetime &closeTime, double &closePrice, double &orderProfit, datetime &lastUpdate, string &comment) {
   ...
   if (Explode(value, ",", values, 12) != 12)              return(!catch("LFX.ReadRemotePosition(1)   invalid .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));

   // Symbol
   sValue = StringTrim(values[0]);
   if (GetCurrencyId(sValue) != LFX.GetCurrencyId(ticket)) return(!catch("LFX.ReadRemotePosition(2)   currency mis-match \""+ GetCurrency(LFX.GetCurrencyId(ticket)) +"/"+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   ...
   else                                                    return(!catch("LFX.ReadRemotePosition(3)   invalid order type \""+ StringTrim(values[1]) +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));

   // OrderUnits
   sValue = StringTrim(values[2]);
   if (!StringIsNumeric(sValue))                           return(!catch("LFX.ReadRemotePosition(4)   invalid unit size \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   double _orderUnits = StrToDouble(sValue);
   if (LE(_orderUnits, 0))                                 return(!catch("LFX.ReadRemotePosition(5)   invalid unit size \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));

   // OpenTime_GMT
   sValue = StringTrim(values[3]);
   if (StringIsDigit(sValue)) datetime _openTime = StrToInteger(sValue);
   else                                _openTime =    StrToTime(sValue);
   if (_openTime <= 0)                                     return(!catch("LFX.ReadRemotePosition(6)   invalid open time \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   _openTime = GMTToServerTime(_openTime);
   if (_openTime > TimeCurrent())                          return(!catch("LFX.ReadRemotePosition(7)   invalid open time \""+ TimeToStr(_openTime, TIME_FULL) +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));

   // OpenPrice
   sValue = StringTrim(values[4]);
   if (!StringIsNumeric(sValue))                           return(!catch("LFX.ReadRemotePosition(8)   invalid open price \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   double _openPrice = StrToDouble(sValue);
   if (LE(_openPrice, 0))                                  return(!catch("LFX.ReadRemotePosition(9)   invalid open price \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));

   // StopLoss
   sValue = StringTrim(values[5]);
   if (!StringIsNumeric(sValue))                           return(!catch("LFX.ReadRemotePosition(10)   invalid stoploss \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   double _stopLoss = StrToDouble(sValue);
   if (LT(_stopLoss, 0))                                   return(!catch("LFX.ReadRemotePosition(11)   invalid stoploss \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));

   // TakeProfit
   sValue = StringTrim(values[6]);
   if (!StringIsNumeric(sValue))                           return(!catch("LFX.ReadRemotePosition(12)   invalid takeprofit \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   double _takeProfit = StrToDouble(sValue);
   if (LT(_takeProfit, 0))                                 return(!catch("LFX.ReadRemotePosition(13)   invalid takeprofit \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   ...
   if      (_closeTime < 0)                                return(!catch("LFX.ReadRemotePosition(14)   invalid close time \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   else if (_closeTime > 0) {
      _closeTime = GMTToServerTime(_closeTime);
      if (_closeTime > TimeCurrent())                      return(!catch("LFX.ReadRemotePosition(15)   invalid close time \""+ TimeToStr(_closeTime, TIME_FULL) +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   }

   // ClosePrice
   sValue = StringTrim(values[8]);
   if (!StringIsNumeric(sValue))                           return(!catch("LFX.ReadRemotePosition(16)   invalid close price \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   double _closePrice = StrToDouble(sValue);
   if (LT(_closePrice, 0))                                 return(!catch("LFX.ReadRemotePosition(17)   invalid close price \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   if (_closeTime==0 && NE(_closePrice, 0))                return(!catch("LFX.ReadRemotePosition(18)   close time/price mis-match 0/"+ NumberToStr(_closePrice, ".+") +" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   if (_closeTime!=0 && EQ(_closePrice, 0))                return(!catch("LFX.ReadRemotePosition(19)   close time/price mis-match "+ TimeToStr(_closeTime, TIME_FULL) +"/0 in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));

   // Profit
   sValue = StringTrim(values[9]);
   if (!StringIsNumeric(sValue))                           return(!catch("LFX.ReadRemotePosition(20)   invalid order profit \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   ...
   if (_lastUpdate <= 0)                                   return(!catch("LFX.ReadRemotePosition(21)   invalid last update time \""+ sValue +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
   _lastUpdate = GMTToServerTime(_lastUpdate);
   if (_lastUpdate > TimeCurrent())                        return(!catch("LFX.ReadRemotePosition(22)   invalid last update time \""+ TimeToStr(_lastUpdate, TIME_FULL) +"\" in .ini value ["+ account +"]->"+ ticket +" = \""+ value +"\" in \""+ file +"\"", ERR_INVALID_CONFIG_PARAMVALUE));
