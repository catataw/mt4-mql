...
string LFX_ORDER.toStr(/*LFX_ORDER*/int lo[], bool debugOutput=false) {
   ...
   if (dimensions == 1) {
      ...
      line        = StringConcatenate("{ticket="        ,                    lo.Ticket        (lo),
                                     ...
                                     ", openPriceLfx="  ,        NumberToStr(lo.OpenPriceLfx  (lo), priceFormat),
                                     ...
                                     ", stopLossLfx="   ,           ifString(lo.StopLossLfx   (lo), NumberToStr(lo.StopLossLfx(lo), priceFormat), "0"),
                                     ", stopLossTime="  ,           ifString(lo.StopLossTime  (lo), "'"+ TimeToStr(lo.StopLossTime(lo), TIME_FULL) +"'", "0"),
                                     ", takeProfitLfx=" ,           ifString(lo.TakeProfitLfx (lo), NumberToStr(lo.TakeProfitLfx(lo), priceFormat), "0"),
                                     ...
                                     ", closePriceLfx=" ,           ifString(lo.ClosePriceLfx (lo), NumberToStr(lo.ClosePriceLfx(lo), priceFormat), "0"),
      ...
      for (int i=0; i < size; i++) {
         ...
         line        = StringConcatenate("[", i, "]={ticket="        ,                    los.Ticket        (lo, i),
                                                  ...
                                                  ", openPriceLfx="  ,        NumberToStr(los.OpenPriceLfx  (lo, i), priceFormat),
                                                  ", openPriceTime=" ,           ifString(los.OpenPriceTime (lo, i), "'"+ TimeToStr(los.OpenPriceTime(lo, i), TIME_FULL) +"'", "0"),
                                                  ", stopLossLfx="   ,           ifString(los.StopLossLfx   (lo, i), NumberToStr(los.StopLossLfx(lo, i), priceFormat), "0"),
                                                  ", stopLossTime="  ,           ifString(los.StopLossTime  (lo, i), "'"+ TimeToStr(los.StopLossTime(lo, i), TIME_FULL) +"'", "0"),
                                                  ", takeProfitLfx=" ,           ifString(los.TakeProfitLfx (lo, i), NumberToStr(los.TakeProfitLfx(lo, i), priceFormat), "0"),
                                                  ...
                                                  ", closePriceLfx=" ,           ifString(los.ClosePriceLfx (lo, i), NumberToStr(los.ClosePriceLfx(lo, i), priceFormat), "0"),
...
int LFX.GetOrder(int ticket, /*LFX_ORDER*/int lo[]) {
   ...
   //Ticket = Symbol, Label, OrderType, OrderUnits, OpenTime, OpenEquity, OpenPrice, OpenPriceTime, StopLoss, StopLossTime, TakeProfit, TakeProfitTime, CloseTime, ClosePrice, OrderProfit, Deviation, Version
   ...
   double _deviation = StrToDouble(sValue);
   ...
   lo.setDeviation     (lo, _deviation     );                        // LFX-Deviation immer vor allen Preisen
...
bool LFX.SaveOrder(/*LFX_ORDER*/int los[], int index=NULL) {
   ...
   string sOpenPrice      =              DoubleToStr(lo.OpenPriceLfx  (lo), lo.Digits(lo));                                                                               sOpenPrice      = StringLeftPad (sOpenPrice     ,  9, " ");
   ...
   string sStopLoss       =                ifString(!lo.StopLossLfx   (lo), "0", DoubleToStr(lo.StopLossLfx(lo),   lo.Digits(lo)));                                       sStopLoss       = StringLeftPad (sStopLoss      ,  8, " ");
   ...
   string sTakeProfit     =                ifString(!lo.TakeProfitLfx (lo), "0", DoubleToStr(lo.TakeProfitLfx(lo), lo.Digits(lo)));                                       sTakeProfit     = StringLeftPad (sTakeProfit    , 10, " ");
   ...
   string sClosePrice     =                ifString(!lo.ClosePriceLfx (lo), "0", DoubleToStr(lo.ClosePriceLfx(lo), lo.Digits(lo)));                                       sClosePrice     = StringLeftPad (sClosePrice    , 10, " ");
   ...
   string section = StringConcatenate(lfxAccountCompany, ".", lfxAccount);
   string key     = ticket;
   string value   = StringConcatenate(sSymbol, ", ", sLabel, ", ", sOperationType, ", ", sUnits, ", ", sOpenTime, ", ", sOpenEquity, ", ", sOpenPrice, ", ", sOpenPriceTime, ", ", sStopLoss, ", ", sStopLossTime, ", ", sTakeProfit, ", ", sTakeProfitTime, ", ", sCloseTime, ", ", sClosePrice, ", ", sProfit, ", ", sDeviation, ", ", sVersion);
