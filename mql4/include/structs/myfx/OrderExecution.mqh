...
string   oe.setSymbol          (/*ORDER_EXECUTION*/int  oe[],          string   symbol    ) {
   if (!StringLen(symbol))                    return(_emptyStr(catch("oe.setSymbol(1)  invalid parameter symbol = "+ StringToStr(symbol), ERR_INVALID_PARAMETER)));
   if (StringLen(symbol) > MAX_SYMBOL_LENGTH) return(_emptyStr(catch("oe.setSymbol(2)  too long parameter symbol = \""+ symbol +"\" (max "+ MAX_SYMBOL_LENGTH +" chars)", ERR_INVALID_PARAMETER)));
...
string   oe.setComment         (/*ORDER_EXECUTION*/int  oe[],          string   comment   ) {
   ...
   if ( StringLen(comment) > 27) return(_emptyStr(catch("oe.setComment()  too long parameter comment = \""+ comment +"\" (max 27 chars)"), ERR_INVALID_PARAMETER));
...
string   oes.setSymbol         (/*ORDER_EXECUTION*/int  oe[][], int i, string   symbol    ) {
   if (!StringLen(symbol))                    return(_emptyStr(catch("oes.setSymbol(1)  invalid parameter symbol = "+ StringToStr(symbol)), ERR_INVALID_PARAMETER));
   if (StringLen(symbol) > MAX_SYMBOL_LENGTH) return(_emptyStr(catch("oes.setSymbol(2)  too long parameter symbol = \""+ symbol +"\" (max "+ MAX_SYMBOL_LENGTH +" chars)"), ERR_INVALID_PARAMETER));
...
string   oes.setComment        (/*ORDER_EXECUTION*/int  oe[][], int i, string   comment   ) {
   if (!StringLen(comment)) comment = "";                            // sicherstellen, daß der String initialisiert ist
   if ( StringLen(comment) > 27) return(_emptyStr(catch("oes.setComment()  too long parameter comment = \""+ comment +"\" (max 27 chars)"), ERR_INVALID_PARAMETER));
...
string ORDER_EXECUTION.toStr(/*ORDER_EXECUTION*/int oe[], bool outputDebug=false) {
   ...
   if (dimensions > 2)                                          return(_emptyStr(catch("ORDER_EXECUTION.toStr(1)  too many dimensions of parameter oe = "+ dimensions, ERR_INVALID_PARAMETER)));
   if (ArrayRange(oe, dimensions-1) != ORDER_EXECUTION.intSize) return(_emptyStr(catch("ORDER_EXECUTION.toStr(2)  invalid size of parameter oe ("+ ArrayRange(oe, dimensions-1) +")", ERR_INVALID_PARAMETER)));
