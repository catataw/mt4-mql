...
string   lo.setComment             (/*LFX_ORDER*/int &lo[],          string   comment            ) {
   ...
   if (StringLen(comment) > 31) return(_emptyStr(catch("lo.setComment()  too long parameter comment = \""+ comment +"\" (maximum 31 chars)"), ERR_INVALID_PARAMETER));
...
string   los.setComment            (/*LFX_ORDER*/int &lo[][], int i, string   comment            ) {
   if (!StringLen(comment)) comment = "";                            // sicherstellen, daß der String initialisiert ist
   if ( StringLen(comment) > 31) return(_emptyStr(catch("los.setComment()  too long parameter comment = \""+ comment +"\" (maximum 31 chars)"), ERR_INVALID_PARAMETER));
...
string LFX_ORDER.toStr(/*LFX_ORDER*/int lo[], bool outputDebug=false) {
   ...
   if (dimensions > 2)                                    return(_emptyStr(catch("LFX_ORDER.toStr(1)  too many dimensions of parameter lo = "+ dimensions, ERR_INVALID_PARAMETER)));
   if (ArrayRange(lo, dimensions-1) != LFX_ORDER.intSize) return(_emptyStr(catch("LFX_ORDER.toStr(2)  invalid size of parameter lo ("+ ArrayRange(lo, dimensions-1) +")", ERR_INVALID_PARAMETER)));
