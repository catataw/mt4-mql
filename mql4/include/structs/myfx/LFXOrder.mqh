 ...
 *    BOOL   modified               //   4         lo[29]      // virtuell: ob die aktuelle Instanz verändert wurde (dieses Element wird nicht gespeichert)
 * } lo;                            // 120 byte = int[30]
 *
...
#define I_LFX_ORDER.modified             29
...
bool     lo.IsModified          (/*LFX_ORDER*/int lo[]         ) {                                                  return(lo[I_LFX_ORDER.modified           ] != 0);                                                                 LFX_ORDER.toStr(lo); }
//----------------------------------------------------------------------- Helper Functions -----------------------------------------------------------------------------------------------------------------------------------------------------------------
...
bool     los.IsModified         (/*LFX_ORDER*/int lo[][], int i) {                                                  return(lo[i][I_LFX_ORDER.modified           ] != 0);                                                              LFX_ORDER.toStr(lo); }
...
// Setter
int      lo.setTicket              (/*LFX_ORDER*/int &lo[],          int      ticket             ) { int v=ticket;                                                                            if (v!= lo[I_LFX_ORDER.ticket             ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.ticket             ] = v;    return(ticket                  ); LFX_ORDER.toStr(lo); }
int      lo.setType                (/*LFX_ORDER*/int &lo[],          int      type               ) { int v=type;                                                                              if (v!= lo[I_LFX_ORDER.type               ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.type               ] = v;    return(type                    ); LFX_ORDER.toStr(lo); }
double   lo.setUnits               (/*LFX_ORDER*/int &lo[],          double   units              ) { int v=MathRound(units      *  10);                                                       if (v!= lo[I_LFX_ORDER.units              ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.units              ] = v;    return(units                   ); LFX_ORDER.toStr(lo); }
double   lo.setLots                (/*LFX_ORDER*/int &lo[],          double   lots               ) { int v=MathRound(lots       * 100);                                                       if (v!= lo[I_LFX_ORDER.lots               ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.lots               ] = v;    return(lots                    ); LFX_ORDER.toStr(lo); }
double   lo.setOpenEquity          (/*LFX_ORDER*/int &lo[],          double   openEquity         ) { int v=MathRound(openEquity * 100);                                                       if (v!= lo[I_LFX_ORDER.openEquity         ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.openEquity         ] = v;    return(openEquity              ); LFX_ORDER.toStr(lo); }
datetime lo.setOpenTime            (/*LFX_ORDER*/int &lo[],          datetime openTime           ) { int v=openTime;                                                                          if (v!= lo[I_LFX_ORDER.openTime           ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.openTime           ] = v;    return(openTime                ); LFX_ORDER.toStr(lo); }
double   lo.setOpenPriceLfx        (/*LFX_ORDER*/int &lo[],          double   openPriceLfx       ) { int v=MathRound(openPriceLfx  * MathPow(10, lo.Digits(lo)));                             if (v!= lo[I_LFX_ORDER.openPriceLfx       ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.openPriceLfx       ] = v;    return(openPriceLfx            ); LFX_ORDER.toStr(lo); }
datetime lo.setOpenTriggerTime     (/*LFX_ORDER*/int &lo[],          datetime openTriggerTime    ) { int v=openTriggerTime;                                                                   if (v!= lo[I_LFX_ORDER.openTriggerTime    ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.openTriggerTime    ] = v;    return(openTriggerTime         ); LFX_ORDER.toStr(lo); }
double   lo.setStopLossLfx         (/*LFX_ORDER*/int &lo[],          double   stopLossLfx        ) { int v=MathRound(stopLossLfx   * MathPow(10, lo.Digits(lo)));                             if (v!= lo[I_LFX_ORDER.stopLossLfx        ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.stopLossLfx        ] = v;    return(stopLossLfx             ); LFX_ORDER.toStr(lo); }
double   lo.setStopLossValue       (/*LFX_ORDER*/int &lo[],          double   stopLossValue      ) { int v=EMPTY_VALUE; if (stopLossValue  !=EMPTY_VALUE) v=MathRound(stopLossValue   * 100); if (v!= lo[I_LFX_ORDER.stopLossValue      ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.stopLossValue      ] = v;    return(stopLossValue           ); LFX_ORDER.toStr(lo); }
bool     lo.setStopLossTriggered   (/*LFX_ORDER*/int &lo[],          bool     stopLossTriggered  ) { int v=stopLossTriggered != 0;                                                            if (v!=(lo[I_LFX_ORDER.stopLossTriggered  ]!=0))    lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.stopLossTriggered  ] = v;    return(stopLossTriggered != 0  ); LFX_ORDER.toStr(lo); }
double   lo.setTakeProfitLfx       (/*LFX_ORDER*/int &lo[],          double   takeProfitLfx      ) { int v=MathRound(takeProfitLfx * MathPow(10, lo.Digits(lo)));                             if (v!= lo[I_LFX_ORDER.takeProfitLfx      ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.takeProfitLfx      ] = v;    return(takeProfitLfx           ); LFX_ORDER.toStr(lo); }
double   lo.setTakeProfitValue     (/*LFX_ORDER*/int &lo[],          double   takeProfitValue    ) { int v=EMPTY_VALUE; if (takeProfitValue!=EMPTY_VALUE) v=MathRound(takeProfitValue * 100); if (v!= lo[I_LFX_ORDER.takeProfitValue    ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.takeProfitValue    ] = v;    return(takeProfitValue         ); LFX_ORDER.toStr(lo); }
bool     lo.setTakeProfitTriggered (/*LFX_ORDER*/int &lo[],          bool     takeProfitTriggered) { int v=takeProfitTriggered != 0;                                                          if (v!=(lo[I_LFX_ORDER.takeProfitTriggered]!=0))    lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.takeProfitTriggered] = v;    return(takeProfitTriggered != 0); LFX_ORDER.toStr(lo); }
datetime lo.setCloseTriggerTime    (/*LFX_ORDER*/int &lo[],          datetime closeTriggerTime   ) { int v=closeTriggerTime;                                                                  if (v!= lo[I_LFX_ORDER.closeTriggerTime   ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.closeTriggerTime   ] = v;    return(closeTriggerTime        ); LFX_ORDER.toStr(lo); }
datetime lo.setCloseTime           (/*LFX_ORDER*/int &lo[],          datetime closeTime          ) { int v=closeTime;                                                                         if (v!= lo[I_LFX_ORDER.closeTime          ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.closeTime          ] = v;    return(closeTime               ); LFX_ORDER.toStr(lo); }
double   lo.setClosePriceLfx       (/*LFX_ORDER*/int &lo[],          double   closePriceLfx      ) { int v=MathRound(closePriceLfx * MathPow(10, lo.Digits(lo)));                             if (v!= lo[I_LFX_ORDER.closePriceLfx      ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.closePriceLfx      ] = v;    return(closePriceLfx           ); LFX_ORDER.toStr(lo); }
double   lo.setProfit              (/*LFX_ORDER*/int &lo[],          double   profit             ) { int v=MathRound(profit * 100);                                                           if (v!= lo[I_LFX_ORDER.profit             ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.profit             ] = v;    return(profit                  ); LFX_ORDER.toStr(lo); }
double   lo.setDeviation           (/*LFX_ORDER*/int &lo[],          double   deviation          ) { int v=MathRound(deviation     * MathPow(10, lo.Digits(lo)));                             if (v!= lo[I_LFX_ORDER.deviation          ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.deviation          ] = v;    return(deviation               ); LFX_ORDER.toStr(lo); }
string   lo.setComment             (/*LFX_ORDER*/int &lo[],          string   comment            ) {
   ...
   if (StringLen(comment) > 31) return(_empty(catch("lo.setComment()   too long parameter comment = \""+ comment +"\" (maximum 31 chars)"), ERR_INVALID_FUNCTION_PARAMVALUE));                if (comment != lo.Comment(lo))                      lo[I_LFX_ORDER.modified]=1;
...
datetime lo.setModificationTime    (/*LFX_ORDER*/int &lo[],          datetime modificationTime   ) { int v=modificationTime;                                                                  if (v!= lo[I_LFX_ORDER.modificationTime   ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.modificationTime   ] = v;    return(modificationTime        ); LFX_ORDER.toStr(lo); }
int      lo.setVersion             (/*LFX_ORDER*/int &lo[],          int      version            ) { int v=version;                                                                           if (v!= lo[I_LFX_ORDER.version            ])        lo[I_LFX_ORDER.modified]=1;    lo[I_LFX_ORDER.version            ] = v;    return(version                 ); LFX_ORDER.toStr(lo); }
bool     lo.setModified            (/*LFX_ORDER*/int &lo[],          bool     modified           ) { int v=modified != 0;                                                                                                                                                        lo[I_LFX_ORDER.modified           ] = v;    return(modified != 0           ); LFX_ORDER.toStr(lo); }
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
int      los.setTicket             (/*LFX_ORDER*/int &lo[][], int i, int      ticket             ) { int v=ticket;                                                                            if (v!= lo[i][I_LFX_ORDER.ticket             ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.ticket             ] = v; return(ticket                  ); LFX_ORDER.toStr(lo); }
int      los.setType               (/*LFX_ORDER*/int &lo[][], int i, int      type               ) { int v=type;                                                                              if (v!= lo[i][I_LFX_ORDER.type               ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.type               ] = v; return(type                    ); LFX_ORDER.toStr(lo); }
double   los.setUnits              (/*LFX_ORDER*/int &lo[][], int i, double   units              ) { int v=MathRound(units      *  10);                                                       if (v!= lo[i][I_LFX_ORDER.units              ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.units              ] = v; return(units                   ); LFX_ORDER.toStr(lo); }
double   los.setLots               (/*LFX_ORDER*/int &lo[][], int i, double   lots               ) { int v=MathRound(lots       * 100);                                                       if (v!= lo[i][I_LFX_ORDER.lots               ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.lots               ] = v; return(lots                    ); LFX_ORDER.toStr(lo); }
double   los.setOpenEquity         (/*LFX_ORDER*/int &lo[][], int i, double   openEquity         ) { int v=MathRound(openEquity * 100);                                                       if (v!= lo[i][I_LFX_ORDER.openEquity         ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.openEquity         ] = v; return(openEquity              ); LFX_ORDER.toStr(lo); }
datetime los.setOpenTime           (/*LFX_ORDER*/int &lo[][], int i, datetime openTime           ) { int v=openTime;                                                                          if (v!= lo[i][I_LFX_ORDER.openTime           ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.openTime           ] = v; return(openTime                ); LFX_ORDER.toStr(lo); }
double   los.setOpenPriceLfx       (/*LFX_ORDER*/int &lo[][], int i, double   openPriceLfx       ) { int v=MathRound(openPriceLfx  * MathPow(10, los.Digits(lo, i)));                         if (v!= lo[i][I_LFX_ORDER.openPriceLfx       ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.openPriceLfx       ] = v; return(openPriceLfx            ); LFX_ORDER.toStr(lo); }
datetime los.setOpenTriggerTime    (/*LFX_ORDER*/int &lo[][], int i, datetime openTriggerTime    ) { int v=openTriggerTime;                                                                   if (v!= lo[i][I_LFX_ORDER.openTriggerTime    ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.openTriggerTime    ] = v; return(openTriggerTime         ); LFX_ORDER.toStr(lo); }
double   los.setStopLossLfx        (/*LFX_ORDER*/int &lo[][], int i, double   stopLossLfx        ) { int v=MathRound(stopLossLfx   * MathPow(10, los.Digits(lo, i)));                         if (v!= lo[i][I_LFX_ORDER.stopLossLfx        ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.stopLossLfx        ] = v; return(stopLossLfx             ); LFX_ORDER.toStr(lo); }
double   los.setStopLossValue      (/*LFX_ORDER*/int &lo[][], int i, double   stopLossValue      ) { int v=EMPTY_VALUE; if (stopLossValue  !=EMPTY_VALUE) v=MathRound(stopLossValue   * 100); if (v!= lo[i][I_LFX_ORDER.stopLossValue      ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.stopLossValue      ] = v; return(stopLossValue           ); LFX_ORDER.toStr(lo); }
bool     los.setStopLossTriggered  (/*LFX_ORDER*/int &lo[][], int i, bool     stopLossTriggered  ) { int v=stopLossTriggered != 0;                                                            if (v!=(lo[i][I_LFX_ORDER.stopLossTriggered  ]!=0)) lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.stopLossTriggered  ] = v; return(stopLossTriggered != 0  ); LFX_ORDER.toStr(lo); }
double   los.setTakeProfitLfx      (/*LFX_ORDER*/int &lo[][], int i, double   takeProfitLfx      ) { int v=MathRound(takeProfitLfx * MathPow(10, los.Digits(lo, i)));                         if (v!= lo[i][I_LFX_ORDER.takeProfitLfx      ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.takeProfitLfx      ] = v; return(takeProfitLfx           ); LFX_ORDER.toStr(lo); }
double   los.setTakeProfitValue    (/*LFX_ORDER*/int &lo[][], int i, double   takeProfitValue    ) { int v=EMPTY_VALUE; if (takeProfitValue!=EMPTY_VALUE) v=MathRound(takeProfitValue * 100); if (v!= lo[i][I_LFX_ORDER.takeProfitValue    ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.takeProfitValue    ] = v; return(takeProfitValue         ); LFX_ORDER.toStr(lo); }
bool     los.setTakeProfitTriggered(/*LFX_ORDER*/int &lo[][], int i, bool     takeProfitTriggered) { int v=takeProfitTriggered != 0;                                                          if (v!=(lo[i][I_LFX_ORDER.takeProfitTriggered]!=0)) lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.takeProfitTriggered] = v; return(takeProfitTriggered != 0); LFX_ORDER.toStr(lo); }
datetime los.setCloseTriggerTime   (/*LFX_ORDER*/int &lo[][], int i, datetime closeTriggerTime   ) { int v=closeTriggerTime;                                                                  if (v!= lo[i][I_LFX_ORDER.closeTriggerTime   ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.closeTriggerTime   ] = v; return(closeTriggerTime        ); LFX_ORDER.toStr(lo); }
datetime los.setCloseTime          (/*LFX_ORDER*/int &lo[][], int i, datetime closeTime          ) { int v=closeTime;                                                                         if (v!= lo[i][I_LFX_ORDER.closeTime          ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.closeTime          ] = v; return(closeTime               ); LFX_ORDER.toStr(lo); }
double   los.setClosePriceLfx      (/*LFX_ORDER*/int &lo[][], int i, double   closePriceLfx      ) { int v=MathRound(closePriceLfx * MathPow(10, los.Digits(lo, i)));                         if (v!= lo[i][I_LFX_ORDER.closePriceLfx      ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.closePriceLfx      ] = v; return(closePriceLfx           ); LFX_ORDER.toStr(lo); }
double   los.setProfit             (/*LFX_ORDER*/int &lo[][], int i, double   profit             ) { int v=MathRound(profit * 100);                                                           if (v!= lo[i][I_LFX_ORDER.profit             ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.profit             ] = v; return(profit                  ); LFX_ORDER.toStr(lo); }
double   los.setDeviation          (/*LFX_ORDER*/int &lo[][], int i, double   deviation          ) { int v=MathRound(deviation     * MathPow(10, los.Digits(lo, i)));                         if (v!= lo[i][I_LFX_ORDER.deviation          ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.deviation          ] = v; return(deviation               ); LFX_ORDER.toStr(lo); }
string   los.setComment            (/*LFX_ORDER*/int &lo[][], int i, string   comment            ) {
   if (!StringLen(comment)) comment = "";                            // sicherstellen, daß der String initialisiert ist
   if ( StringLen(comment) > 31) return(_empty(catch("los.setComment()   too long parameter comment = \""+ comment +"\" (maximum 31 chars)"), ERR_INVALID_FUNCTION_PARAMVALUE));              if (comment != los.Comment(lo, i))                  lo[i][I_LFX_ORDER.modified]=1;
...
datetime los.setModificationTime   (/*LFX_ORDER*/int &lo[][], int i, datetime modificationTime   ) { int v=modificationTime;                                                                  if (v!= lo[i][I_LFX_ORDER.modificationTime   ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.modificationTime   ] = v; return(modificationTime        ); LFX_ORDER.toStr(lo); }
int      los.setVersion            (/*LFX_ORDER*/int &lo[][], int i, int      version            ) { int v=version;                                                                           if (v!= lo[i][I_LFX_ORDER.version            ])     lo[i][I_LFX_ORDER.modified]=1; lo[i][I_LFX_ORDER.version            ] = v; return(version                 ); LFX_ORDER.toStr(lo); }
bool     los.setModified           (/*LFX_ORDER*/int &lo[][], int i, bool     modified           ) { int v=modified != 0;                                                                                                                                                        lo[i][I_LFX_ORDER.modified           ] = v; return(modified != 0           ); LFX_ORDER.toStr(lo); }
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
string LFX_ORDER.toStr(/*LFX_ORDER*/int lo[], bool debugger=false) {
   ...
   if (dimensions == 1) {
      ...
      line        = StringConcatenate("{ticket="             ,                    lo.Ticket             (lo),
                                     ...
                                     ", modified="           ,          BoolToStr(lo.IsModified         (lo)), "}");
      if (debugger)
      ...
      for (int i=0; i < size; i++) {
         ...
         line        = StringConcatenate("[", i, "]={ticket="             ,                    los.Ticket             (lo, i),
                                                  ...
                                                  ", modified="           ,          BoolToStr(los.IsModified         (lo, i)), "}");
         if (debugger)
            debug("LFX_ORDER.toStr()   "+ line);
         ArrayPushString(lines, line);
      }
   ...
   lo.IsModified            (iNulls);       los.IsModified            (iNulls, NULL);
   ...
   lo.setModified           (iNulls, NULL); los.setModified           (iNulls, NULL, NULL);
...
//   bool     lo.IsModified             (/*LFX_ORDER*/int lo[]);
...
//   bool     los.IsModified            (/*LFX_ORDER*/int lo[][], int i);
...
//   bool     lo.setModified            (/*LFX_ORDER*/int lo[], bool     modified           );
...
//   bool     los.setModified           (/*LFX_ORDER*/int lo[][], int i, bool     modified           );
