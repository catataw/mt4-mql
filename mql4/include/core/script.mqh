...
int init() {
   ...
   if (_bool(initFlags & INIT_PIPVALUE)) {
      ...
      if (!TickSize)                 return(catch("init(2)   MarketInfo(MODE_TICKSIZE) = "+ NumberToStr(TickSize, ".+"), ERR_INVALID_MARKET_DATA));

      double tickValue = MarketInfo(Symbol(), MODE_TICKVALUE);
      if (IsError(catch("init(3)"))) return(last_error);
      if (!tickValue)                return(catch("init(4)   MarketInfo(MODE_TICKVALUE) = "+ NumberToStr(tickValue, ".+"), ERR_INVALID_MARKET_DATA));
   }
